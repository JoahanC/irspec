...
def sortdata(sorting_method, name, line_center):
    data = ascii.read(paths.spaxs_dir() / name / "twogaussian_raw.dat", format="ipac")  
    ...
    for idx, _ in enumerate(data["XPIX"]):
        ...
        if np.isnan(g3_amp):
            if np.isnan(g2_amp):
                ...
                if sorting_method == "CEN":
                    if np.abs(g1_vel - line_center) < np.abs(g2_vel - line_center):
                        g1_amps.append(g1_amp)
                        g2_amps.append(g2_amp)
                        g3_amps.append(np.nan)
                        g1_vels.append(g1_vel)
        ...
        # ALL IN PLAY
            ...
            if sorting_method == "CEN":
                dict_keys = [np.abs(g1_vel - line_center), np.abs(g2_vel - line_center), np.abs(g3_vel - line_center)]
                sort_dict = {np.abs(g1_vel - line_center): [g1_amp, g1_sigma], np.abs(g2_vel - line_center): [g2_amp, g2_sigma], np.abs(g3_vel - line_center): [g3_amp, g3_sigma]}
                key_dict = {np.abs(g1_vel - line_center): g1_vel, np.abs(g2_vel - line_center): g2_vel, np.abs(g3_vel - line_center): g3_vel}
                dict_keys.sort()
                print(dict_keys)
                g1_amps.append(sort_dict[dict_keys[0]][0])
                g2_amps.append(sort_dict[dict_keys[1]][0])
                g3_amps.append(sort_dict[dict_keys[2]][0])
                g1_vels.append(key_dict[dict_keys[0]])
                g2_vels.append(key_dict[dict_keys[1]])
                g3_vels.append(key_dict[dict_keys[2]])
            ...
            if sorting_method == "AMP":
                ...
                dict_keys.sort(reverse=True)
                print(dict_keys)
                g1_amps.append(dict_keys[0])
                g2_amps.append(dict_keys[1])
                g3_amps.append(dict_keys[2])
    ...
    fitparams.write(paths.spaxs_dir() / name / f"{sorting_method}_sort.dat", format="ipac", overwrite=True)

def main():
    sorting_method = "AMP"
    name = "[ArIII]"
    line_params = read_line_params()
    sortdata(sorting_method, name, line_params[name][4])
