...
def render_multicomponent_plot(data, wcs, savefig=False):
    ...
    title_name = line_name
    if "H_2_S_1" in line_name:
        title_name = r"H$_{2}$ S(1)"
    if "H_2_S_3" in line_name:
    ...
    if savefig:
        plt.savefig(f"./../diagnostic_plots/dynamic_multicomponent/{line_name}/compnum_white.png", dpi=600)
        plt.close()
    else:
        plt.show()
...
def render_totflux_plot(data, wcs, savefig=False):
    ...
    add_scalebar(ax, scalebar_angle, label="5 kpc", color="white", fontproperties=fontprops)
    if savefig:
        plt.savefig(f"./../diagnostic_plots/dynamic_multicomponent/{line_name}/flux_white.png", dpi=600)
        plt.close()
    else:
        plt.show()


def render_amplitude_plot(data, line_name, line_center, wcs, output, param="G1AMP", savefig=False):
    """
    Renders a spaxel map illustrating the number of components fit to 
    each spaxel
    """
    ...
    for idx, _ in enumerate(data["XPIX"]):
        ...
        vel_disp = ((const.c * (np.abs(data[f"G{pref_idx}SIGMA"][idx]))/line_center).to(u.kilometer / u.second)).value
        
        if np.abs(vel_disp) > max_disp + 100:
    ...
    add_scalebar(ax, scalebar_angle, label="5 kpc", color="white", fontproperties=fontprops)
    if savefig:
        plt.savefig(output + f"{param}_white.png", dpi=600)
        plt.close()
    else:
        plt.show()
        
def render_rel_vel_plot(data, line_center, line_name, wcs, output, param="G2CEN", savefig=False):
    """
    Renders a spaxel map illustrating the number of components fit to 
    each spaxel
    """
    ...
    for idx, _ in enumerate(data["XPIX"]):
        ...
        if np.abs(rel_vel) > max_bound + 100:
            base_array[data["XPIX"][idx]][data["YPIX"][idx]] = np.nan
            all_relvels.append(np.nan)
        else:
            if np.abs(vel_disp) > max_disp + 100:
                base_array[data["XPIX"][idx]][data["YPIX"][idx]] = np.nan
            else:
...
def render_vel_disp_plot(data, line_center, line_name, wcs, output, param="G2SIGMA", savefig=False):
    """
    Renders a spaxel map illustrating the number of components fit to 
    each spaxel
    """
    ...
    for idx, _ in enumerate(data["XPIX"]):
        ...
        rel_vel = ((const.c * (data[param.replace("SIGMA", "CEN")][idx] - line_center)/line_center).to(u.kilometer / u.second)).value
...
def render_vel_disp_rel_vel_scatter(data, line_center, line_name, output, savefig=False):
    fig, ax = plt.subplots()
    fig.set_size_inches(10, 8)
    prefixes = ["G1", "G2", "G3"]
    rel_vels = []
    ...
    ax.set_title(title_name, loc="left")
    if savefig:
        plt.savefig(output + f"vel_disp_rel_vel_white.png", dpi=600)
        plt.close()
    else:
        plt.show()
...
"""line_name = "[NeIII]"
line_dict = read_line_params()
data = ascii.read(f"./../diagnostic_plots/dynamic_multicomponent/{line_name}/AMP_SNR_fit.dat", format="ipac")  
