...
#%%   
    def plotGrid(self, cube, user_ra, user_dec, gridstep_dist, gridPointsX, gridPointsY, r_as, output_path, output_filebase_name):
        ...
        #print(sky_list[0])
        #positions = skycoord_to_pixel(sky_list[0], cube.wcs)
        #test_rect = RectangularAperture(positions, 1 / cube.pixel_scale, 1 / cube.pixel_scale)
        #test_rect.plot(ax=plt.gca())
        #plt.gca().add_patch(Rectangle([sky_list[0].ra.value - r_as/3600, sky_list[0].dec.value - r_as/3600], 0.0003, 0.0003, linewidth=1, edgecolor='r', facecolor='none', transform=ax.get_transform('world')))
        ...
        for i in range(len(pixels_list)):
            ...
            positions = skycoord_to_pixel(sky_list[i], cube.wcs)
            test_rect = RectangularAperture(positions, 1 / cube.pixel_scale, 1 / cube.pixel_scale)
            test_rect.plot(ax=plt.gca())
            
            #print(sky_list[i], r_as)
