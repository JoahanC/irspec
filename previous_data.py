...
p = PlotParams(palatte="light", scaling="poster")
...
ppram = PlotParams(palatte="dark", scaling="presentation")
...
nirspec_file1 = "./../archival_data/nircam_images/IR23128/IR23128_f200w.fits"
nirspec_file2 = "./../archival_data/nircam_images/IR23128/IR23128_f356w.fits"
...
data_fixed = interpolate_replace_nans(hdu.data, kernel)
#result = interpolate_replace_nans(image, kernel)

ax.imshow(data_fixed, norm=LogNorm(z1, z2), origin='lower', cmap="plasma")
...
print(z1, z2)
ax.imshow(np.fliplr(np.flipud(data)), norm=LogNorm(0.2, 30), cmap="plasma")
#for idx, name in enumerate(coords):
#    coord_obj = SkyCoord(coords[name][0], coords[name][1], unit="deg")
#    x_pix, y_pix = skycoord_to_pixel(coord_obj, wcs)
#    rad = coords[name][2] / miri_pixscale
...
plt.grid(alpha=0.25)
plt.show()
#plt.savefig("spitzer_8-.pdf", dpi=1000, bbox_inches="tight")"""
#plt.savefig("miri_444_spots.pdf", dpi=1000)

"""wfc_pixescale=0.05
hdu = fits.open(hst_file2, ext=1)[1]
wcs = WCS(hdu.header)
fig = plt.figure()
fig.set_size_inches(6, 6)
ax = plt.subplot(projection=wcs)
...
data = hdu.data[2150:2600, 1900:2400]
print(np.shape(hdu.data))
z1, z2 = z.get_limits(data)
ax.imshow(np.fliplr(np.flipud(data)), norm=LogNorm(0.2, 20), origin='lower', cmap="plasma")"""
...
ax.set_xlabel("RA (J2000)", fontsize=20)
ax.set_ylabel("Dec (J2000)", fontsize=20)
ax.set_title("Hubble WFC F814W", loc="right", fontsize=22)
plt.grid(alpha=0.25)
#plt.show()
plt.savefig("wfc_1.pdf", dpi=1200, bbox_inches='tight')
#plt.close()"""

hdu = fits.open(nirspec_file1, ext=1)[1]
wcs = WCS(hdu.header)
fig = plt.figure()
fig.set_size_inches(6, 6)
ax = plt.subplot(projection=wcs)
z1 = 0.5
z2 = 1000
data = hdu.data[3000:3400, 3400:3800]
z = ZScaleInterval(hdu.data)
#result = interpolate_replace_nans(image, kernel)
print(np.shape(hdu.data))
ax.imshow(data, norm=LogNorm(z1, z2), origin='lower', cmap="plasma")
#for idx, name in enumerate(coords):
#    coord_obj = SkyCoord(coords[name][0], coords[name][1], unit="deg")
#    x_pix, y_pix = skycoord_to_pixel(coord_obj, wcs)
#    rad = coords[name][2] / miri_pixscale
...
ax.set_xlabel("RA (J2000)", fontsize=20)
ax.set_ylabel("Dec (J2000)", fontsize=20)
ax.set_title("NIRCAM F200W", loc="right", fontsize=22)
plt.grid(alpha=0.25)
plt.savefig("nircam_f200w.pdf", dpi=1000, bbox_inches='tight')